import * as applyUpdates from "immutability-helper";
const emptyDefinition = (id) => ({
  id,
  spelling: "",
  announced: false,
  revealed: true,
...
export const newGame = (roomName) => ({
  ...
  chat: [],
  words: [
    // emptyDefinition(uuid()),
    // { ...emptyDefinition(uuid()), spelling: "foo", correctDefinition: "bar" },
    // {
    //   ...emptyDefinition(uuid()),
    //   spelling: "foo2",
...
export const addDefinitionAction = () => ({
  shape: "addDefinition",
});

export const removeDefinitionAction = (id) => ({
  shape: "removeDefinition",
  id,
});

export const updateDefinitionAction = (id, spelling, correctDefinition) => ({
  shape: "updateDefinition",
  id,
  spelling,
  correctDefinition,
});

export const reorderDefinitionsAction = (ids) => ({
  shape: "reorderDefinitions",
  ids,
});
...
export const closeSubmissionsAction = (id, deadline) => ({
  shape: "closeSubmissions",
  id,
  deadline,
});
...
const update = (game, action) => {
  ...
  if (action.shape === "addDefinition") {
    return applyUpdates(game, {
      words: { $push: [emptyDefinition(uuid())] },
    });
  }

  if (action.shape === "updateDefinition") {
    return applyUpdates(game, {
      words: {
  ...
  if (action.shape === "removeDefinition") {
    return applyUpdates(game, {
      words: {
  ...
  if (action.shape === "reorderDefinitions") {
    return applyUpdates(game, {
      words: {
        $apply: (words) =>
          action.ids
  ...
  if (action.shape === "closeSubmissions") {
    return onWord(
      action.id,
